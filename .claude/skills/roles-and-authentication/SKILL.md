...
function ProtectedPage() {
  ...
  useEffect(() => {
    if (!user.isAuthenticated) {
      const loginPageUrl = getPageUrl('Login');
...
- **2+ roles = Multi-role app** — **Every page and component must check `user.role`** to provide the correct experience; no exceptions
- **Multi-role means 2+ DIFFERENT personas** (Buyer AND Seller, Admin AND Member — not just one persona with a label)
...
**When to use:** The app shows each user only content belonging to their entity — client portal, tenant dashboard, patient portal. Telltale sign: a **linkage table** (e.g. `ClientUsers`) mapping user email → entity id. These apps always have **Admin + end-user** roles; the app owner is Admin and manages entities/linkages in-app.
